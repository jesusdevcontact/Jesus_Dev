...
export class TechStackCardComponent {
  ...
  techIcon(item: string): TechIcon {
    const techIconPath = (name: string): TechIcon => ({
      type: 'asset',
      path: `assets/icons/tech/${name}.svg`,
    });

    const icons: Record<string, TechIcon> = {
      angular: techIconPath('angular'),
      typescript: techIconPath('typescript'),
      html5: techIconPath('html5'),
      css3: techIconPath('css3'),
      scss: techIconPath('sass'),
      rxjs: techIconPath('rxjs'),
      react: techIconPath('react'),
      javascript: techIconPath('javascript'),
      laravel: techIconPath('laravel'),
      php: techIconPath('php'),
      filament: { type: 'lucide', name: 'panel-top' },
      python: techIconPath('python'),
      java: techIconPath('java'),
      mysql: techIconPath('mysql'),
      postgresql: techIconPath('postgresql'),
      neon: techIconPath('neon'),
      cypress: techIconPath('cypress'),
      vitest: techIconPath('vitest'),
      phpunit: techIconPath('phpunit'),
      ...
      git: techIconPath('git'),
      github: techIconPath('github'),
      'github-actions': techIconPath('github-actions'),
      docker: techIconPath('docker'),
      linux: techIconPath('linux'),
      ssh: { type: 'lucide', name: 'key-round' },
      vercel: techIconPath('vercel'),
      railway: techIconPath('railway'),
      'github-pages': { type: 'lucide', name: 'globe' },
      'vs-code': techIconPath('visual-studio-code'),
      'visual-studio-code': techIconPath('visual-studio-code'),
      postman: techIconPath('postman'),
      metabase: techIconPath('metabase'),
      'openai-codex': { type: 'lucide', name: 'terminal' },
      'claude-code': techIconPath('claude'),
